...
const updateFavorite = async({ userId, favorites }) => {
    ...
    try {
        await client.connect()
        const favCollection = client.db('finalExam').collection('favorites')
        const updateFavoriteUser = await favCollection.updateOne({ 'userId': userId }, { $set: { 'favorites': favorites } })
        await client.close
...
const deleteFavorite = async({ userId, favorites }) => {
    ...
    try {
        await client.connect()
        const favCollection = client.db('finalExam').collection('favorites')
        const deleteFavoriteUser = await favCollection.deleteOne({ 'userId': userId }, { $unset: { "favorites": favorites } })
        await client.close
...
module.exports = { addFavorite, getFavorite, getAllFavorites, updateFavorite, deleteFavorite }